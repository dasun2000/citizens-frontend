  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px',justifyContent: 'center',flexWrap: 'wrap'}}>
        <button onClick={() => setActiveTab('country')}
          style={{padding: '10px 20px',backgroundColor: activeTab === 'country' ? '#007bff' : '#f8f9fa',color: activeTab === 'country' ? 'white' : '#333',border: '1px solid #ddd',borderRadius: '5px',cursor: 'pointer'}}>
          Countries ({data.countries.length})
        </button>
        <button onClick={() => setActiveTab('territory')}
          style={{padding: '10px 20px',backgroundColor: activeTab === 'territory' ? '#007bff' : '#f8f9fa',color: activeTab === 'territory' ? 'white' : '#333',border: '1px solid #ddd',borderRadius: '5px',cursor: 'pointer'}}>
          Territories ({data.territories.length})
        </button>
        
        <button onClick={() => setActiveTab('district')}
          style={{padding: '10px 20px',backgroundColor: activeTab === 'district' ? '#007bff' : '#f8f9fa',color: activeTab === 'district' ? 'white' : '#333',border: '1px solid #ddd',borderRadius: '5px',cursor: 'pointer'}}>
          Districts ({data.districts.length})
        </button>
        
        <button onClick={() => setActiveTab('seat')}
          style={{padding: '10px 20px',backgroundColor: activeTab === 'seat' ? '#007bff' : '#f8f9fa',color: activeTab === 'seat' ? 'white' : '#333',border: '1px solid #ddd',borderRadius: '5px',cursor: 'pointer'}}>
      ...
      <div style={{ marginBottom: '30px' }}>
        ...
        {currentData.length > 10 && (
          <p style={{ textAlign: 'center', color: '#666', fontSize: '12px' }}>
            *Showing top 10 results. See table below for all data.
          </p>
        )}
      </div>
            ...
            <tbody>
              {currentData
                ...
                <tr key={index} style={{ 
                  ...
                  <td style={{ padding: '12px', border: '1px solid #ddd', textAlign: 'center',fontWeight: 'bold',color: item.count > 0 ? '#28a745' : '#6c757d'}}>
                    {item.count}
                  </td>
                  <td style={{ padding: '12px', border: '1px solid #ddd' }}>
                    {item.parent}
